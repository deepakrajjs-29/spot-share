...
export const CTAButton: React.FC<CTAButtonProps> = ({
  ...
  const baseStyles = "inline-flex items-center justify-center font-semibold rounded-lg transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 relative select-none tracking-wide text-center";

  const sizeStyles = {
    sm: "px-3.5 py-1.5 text-xs gap-1.5 min-h-[36px]",
    md: "px-4.5 py-2 text-sm gap-2 min-h-[42px]",
  ...
  const renderIcon = () => {
    switch (icon) {
      case 'arrow':
        return <ArrowRight className="w-4 h-4 ml-0.5" />;
      case 'external':
        return <ExternalLink className="w-3.5 h-3.5 ml-0.5" />;
      case 'download':
        return <Download className="w-4 h-4 ml-0.5" />;
      case 'file':
        return <FileText className="w-4 h-4 ml-0.5" />;
      default:
        return null;
    }
  };
